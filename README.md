...
A **Hotkey Management** entry is added to the top-level Options menu, right before **Settings**. It contains:
...
- A debug-logging toggle (**Enabled**/**Disabled**), gating both this mod's own log output and every registered MD library's `debug_text` calls that opt into it (see `GetDebugChance` below).
